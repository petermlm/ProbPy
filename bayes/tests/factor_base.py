...
class FactorBase:
    def __init__(self):
        ...
        self.XYZ_factor = factor.Factor(\
                [self.X, self.Y, self.Z], list(range(1, 9)))
        self.XYW_factor = factor.Factor(\
                [self.X, self.Y, self.W], list(range(9, 17)))
        self.XKW_factor = factor.Factor(\
                [self.X, self.K, self.W], list(range(17, 25)))
        self.TKW_factor = factor.Factor(\
                [self.T, self.K, self.W], list(range(25, 33)))

        # Factors for normalization
        self.X_factor_n = factor.Factor([self.X], [1, 2])
        self.XY_factor_n = factor.Factor([self.X, self.Y], [1, 1, 2, 2])
        self.XYZ_factor_n = factor.Factor([self.X, self.Y, self.Z], [1, 1, 2, 2, 3, 3, 4, 4])
