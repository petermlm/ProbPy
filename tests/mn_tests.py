...
class TestMarkovNetwork:
    def __init__(self):
        ...
        self.f_v3_v4_v5_v6 = Factor([self.v3, self.v4, self.v5, self.v6], list(range(1, 17)))
        self.f_v6_v7_v8 = Factor([self.v6, self.v7, self.v8], list(range(1, 9)))

    def compare_results(self, bf, MN):
        for i in bf.rand_vars:
            print(bf.marginal(i))
            print(MN.var_nodes[i.name].marginal)
            assert(bf.marginal(i) == MN.var_nodes[i.name].marginal)
    ...
    def mn_test_7(self):
        ...
        # Make belief propagation
        MN = MarkovNetwork([self.f_v3_v4_v5, self.f_v3_v4_v5_v6, self.f_v6_v7_v8])
        MN.BeliefPropagation()
