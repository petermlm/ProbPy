...
class TestTextBook:
	def sprinkler_test_0(self):
		'''
		Simple network from "An introduction to graphical models" by Kevin P. Murphy
		C -> S
		C -> R
		S -> W
		R -> W
		...
		SprinklerNW = bn.BayesianNetwork([
			(C, Cf, []),
			(S, Sf, [C]),
			(R, Rf, [C]),
			(W, Wf, [S,R]) ])
		...
		for (qvar, qvalue, evidence, expect) in cases:
			...
			print("qvar: %s\nevidence: %s\nexpect: %s\nresult: %s"%(qvar.name,",".join(["%s=%s"%(x[0].name,x[1]) for x in evidence]),expect,result.values[qvalue]))
			assert_almost_equal(result.values[qvalue], expect, places = 3)


	def student_test_0(self):
		'''
		Student network from "Probabilistic graphical models principles and techniques" by Koller and Friedman
		D -> G
		I -> G
		I -> S
		G -> L
