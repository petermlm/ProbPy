...
class TestFactorMult(TestBase):
    def div_test_0(self):
        ...
        res = [self.X_factor.div(self.scalar), self.X_factor.div(self.scalarf)]

        for i in res:
            assert i.rand_vars == [self.X] and i.values == [0.1, 0.2]

        res = self.scalarf.div(self.X_factor)
        assert res.rand_vars == [self.X] and res.values == [10 / 1, 10 / 2]
    ...
    def div_test_1(self):
        ...
        res = [self.XY_factor.div(self.scalar), self.XY_factor.div(self.scalarf)]

        for i in res:
            assert i.rand_vars == [self.X, self.Y] and i.values == [0.1, 0.2, 0.3, 0.4]

        res = self.scalarf.div(self.XY_factor)
        assert res.rand_vars == [self.X, self.Y] and res.values == [
            10 / 1,
            10 / 2,
            10 / 3,
            10 / 4,
    ...
    def div_test_2(self):
        ...
        res = [self.XYZ_factor.div(self.scalar), self.XYZ_factor.div(self.scalarf)]
    ...
    def div_test_3(self):
        ...
        assert res.rand_vars == [self.X] and res.values == [1, 1]
    ...
    def div_test_8(self):
        ...
        assert res.rand_vars == [self.X, self.Y] and res.values == [1, 1, 1, 1]
    ...
    def div_test_10(self):
        ...
        # fmt: off
        assert res.values == [1 / 9,  2 / 9,  3 / 9,  4 / 9,
                              1 / 10, 2 / 10, 3 / 10, 4 / 10,
                              1 / 11, 2 / 11, 3 / 11, 4 / 11,
                              1 / 12, 2 / 12, 3 / 12, 4 / 12]
        # fmt: on
    ...
    def div_test_12(self):
        ...
        # fmt: off
        assert res.values == [1 / 9, 2  / 10, 3 / 11, 4 / 12,
                              5 / 9, 6  / 10, 7 / 11, 8 / 12,
                              1 / 13, 2 / 14, 3 / 15, 4 / 16,
                              5 / 13, 6 / 14, 7 / 15, 8 / 16]
        # fmt: on
    ...
    def div_test_13(self):
        ...
        # fmt: off
        assert res.values == [1 / 17, 2 / 18, 3 / 17, 4 / 18,
                              5 / 17, 6 / 18, 7 / 17, 8 / 18,
                              1 / 19, 2 / 20, 3 / 19, 4 / 20,
                              5 / 19, 6 / 20, 7 / 19, 8 / 20,
    ...
    def div_test_14(self):
        ...
        # fmt: off
        assert res.values == [1 / 25, 2 / 25, 3 / 25, 4 / 25,
                              5 / 25, 6 / 25, 7 / 25, 8 / 25,
                              1 / 26, 2 / 26, 3 / 26, 4 / 26,
                              5 / 26, 6 / 26, 7 / 26, 8 / 26,
